...
Apple omits notification endpoints from its default response. Request them using
`--fields` to verify the configured URLs and payload versions.

<ParamField path="--fields" type="string">
  Comma-separated app attributes: `name`, `bundleId`, `sku`, `primaryLocale`,
  `contentRightsDeclaration`, `subscriptionStatusUrl`, `subscriptionStatusUrlVersion`,
...
<ParamField path="--age-rating-fields" type="string">
  ...
  `socialMedia`, `socialMediaAgeRestricted`, `gracRatingClassificationNumber`.
  Automatically includes `ageRatingDeclaration`.
</ParamField>

```bash  theme={null}
asc apps info list --app "APP_ID" --fields kidsAgeBand
asc apps info list --app "APP_ID" --age-rating-fields socialMedia,socialMediaAgeRestricted,gracRatingClassificationNumber
```
...
Update app metadata and App Store Server Notifications URLs.
...
<ParamField path="--subscription-status-url" type="string">
  Production App Store Server Notifications HTTPS URL. Sets notification version V2.
</ParamField>

<ParamField path="--sandbox-subscription-status-url" type="string">
  Sandbox App Store Server Notifications HTTPS URL. Sets notification version V2.
</ParamField>

Notification URLs must be absolute HTTPS URLs without credentials or fragments.
Each supplied URL is configured for version V2, which delivers signed payloads.
Omitting an endpoint leaves its URL and version unchanged. Empty URLs are rejected;
these flags do not remove existing endpoints. Use `asc apps view --id "APP_ID" --fields "subscriptionStatusUrl,subscriptionStatusUrlVersion,subscriptionStatusUrlForSandbox,subscriptionStatusUrlVersionForSandbox" --output json` to read back the configured URLs and versions.

...
* `asc web apps history` records live per app store version; without `--version-id` the command lists the app's versions and reads each version's status changes. Both reads follow Apple's pagination links internally, so there is no `--paginate` flag. Viewing history is gated by the App Status History role capability

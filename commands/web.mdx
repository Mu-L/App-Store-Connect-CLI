...
and expired cookies are dropped. A bundle with no usable cookie is refused
without touching the cache. Importing does not contact Apple, so run
`asc web auth status` afterwards to confirm the session is still accepted.

...
* `asc web apps declarations list` is read-only and reports each requirement with its status; a requirement still at `PENDING_COLLECTION` blocks App Store submission
...
* Declarations other than the regulated medical device one, including the personal-service declaration and EU DSA trader status, are still website-only; `asc web apps declarations list` shows their status without answering them

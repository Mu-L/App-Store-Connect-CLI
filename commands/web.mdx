...
The read commands use the cookie-authenticated Developer Portal JSON:API
service, not the public App Store Connect API. `list` requests the first
1000-resource iOS/Mac collection and does not expose pagination; if Apple
returns `links.next`, it remains visible in JSON for a later slice. `view`
accepts the opaque resource ID returned by `list` and includes the requested
Bundle ID capability graph. `get` remains accepted as a deprecated compatibility
alias with a warning. Both commands are read-only and do not require
`--confirm`.

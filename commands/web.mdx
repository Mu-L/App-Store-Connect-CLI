...
* Use `asc web subscriptions pricing adjusted-equalizations view` when you need
  the web-session `/iris` MONTHLY price matrix or Apple's normalized
  `missingTerritories` diagnostic after an equalization failure. This workflow
  requires a cached web session (`asc web auth login`) and accepts the
  workflow-specific `--price-point-id` and `--plan-type MONTHLY` inputs. It
  prints a computed JSON result rather than Apple's public JSON:API envelope.

...
them, are rejected. The file is written atomically with mode `0600`; an
existing file is not replaced unless `--overwrite` is set. The receipt reports
the path, bytes written, content type, and agreement ID. The download URL and
the file contents are never printed.

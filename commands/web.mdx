...
Import validates the bundle before writing anything: the document kind and
...
account. Import contacts Apple's session endpoint and refuses invalid,
expired, unreachable, malformed, or identity-mismatched sessions before any
cache write. That check is not optional: without it a stale or hostile bundle
would be cached under whatever Apple Account it claims, and a machine that
cannot reach Apple could not use the imported session anyway. Run
`asc web auth status` afterwards to inspect the resumed session if needed.

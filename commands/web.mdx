...
IDs on stderr. Remove those assignments with `unassign` or `set` first. Every
mutation verifies its result by re-reading the Bundle ID or the team's App Group

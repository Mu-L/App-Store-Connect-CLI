...
The local screenshot matrix workflow is experimental. If you encounter issues, please [file feedback](https://github.com/rorkai/App-Store-Connect-CLI/issues/new/choose).
...
explicit same-family mapping accepted by the frame-device parser. Use
`--max-attempts` and `--retry-backoff` to override the plan. Retries apply only
to execution and framing; validation, cancellation, and cleanup failures are
reported without retrying. See [the matrix design note](https://github.com/rorkai/App-Store-Connect-CLI/blob/main/docs/design/screenshots-matrix.md)

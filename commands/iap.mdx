...
resource. Localizations, images, and review submission are all version-scoped:
resolve or create an IAP version first, then pass its ID to the commands below.
...
Upload a promotional image for an in-app purchase version. Create or resolve an
IAP version first; images belong to the version, not the product.
...
# Update an in-progress upload using the existing screenshot ID
asc iap review-screenshots update \
  --screenshot-id "SCREENSHOT_ID" \
  --file "./replacement.png"

# A completed screenshot cannot be replaced by update. Delete it explicitly,
# then create the replacement (the delete command requires --confirm).
...
# Finalize an upload reservation with the API-supported PATCH attributes
asc iap review-screenshots update \
  --screenshot-id "SCREENSHOT_ID" \
  --uploaded true \
  --checksum "MD5_HASH"

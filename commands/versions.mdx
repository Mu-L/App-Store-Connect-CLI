...
Read the current value with `asc versions list --output json` or `asc versions view --output json`. Apple omits `downloadable` for versions that never carried the setting, so an absent key means Apple did not report the attribute.

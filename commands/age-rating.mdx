...
`--fields` accepts `gracRatingClassificationNumber`, `socialMedia`, and
`socialMediaAgeRestricted`. It is also
...
## Korea Ratings (API 4.5)

`--korea-age-rating-override` accepts `NONE`, `ALL`, `TWELVE_PLUS`,
`FIFTEEN_PLUS`, and `NINETEEN_PLUS`. Submission validation accepts the same values.
Use `--grac-rating-classification-number` to set the GRAC classification number:

```bash
asc age-rating edit --id "DECLARATION_ID" --korea-age-rating-override TWELVE_PLUS --grac-rating-classification-number "CLASSIFICATION_NUMBER"
```

The classification number must be nonempty. Omit the flag to leave it unchanged.
Declaration responses include the number in JSON, table, and Markdown output.

Read the GRAC number directly or through an included age-rating declaration:

```bash
asc age-rating view --app-info-id "APP_INFO_ID" --fields gracRatingClassificationNumber
asc apps info view --info-id "APP_INFO_ID" --age-rating-fields gracRatingClassificationNumber
asc apps info list --app "APP_ID" --age-rating-fields gracRatingClassificationNumber
```

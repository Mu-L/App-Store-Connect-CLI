...
deprecate the app-tag `territories` relationship and both territory endpoints.
Territory-related list parameters are also deprecated. The published OpenAPI
snapshot still exposes these operations and parameters.

For compatibility, the CLI continues forwarding existing requests and emits
one warning on stderr per invocation. This includes `list` and `view` territory
selections, `territories`, `territories-links`, and territory parameters in
continuation URLs. JSON stdout, pagination, and API errors are unchanged.

Use `asc app-tags list --app "APP_ID"` or `view` without territory selections
for core tag data. Remove `territories` from `--fields` and omit `--include
territories`, `--territory-fields`, and `--territory-limit`. Apple does not name
a replacement for territory lookups in these release notes. Tag linkage IDs
(`links`) and visibility updates remain supported without a deprecation warning.
...
* `territories` - the legacy storefront relationship, deprecated in API 4.5
...
* `territories` returns full territory resources linked to a tag (deprecated).
* `territories-links` returns territory relationship IDs only (deprecated).

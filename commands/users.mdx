...
* `--visible-app` - [experimental] Filter by visible app ID(s), comma-separated
* `--sort` - [experimental] Sort by `username`, `-username`, `lastName`, or `-lastName`
* `--fields` - [experimental] User fields to include: `username`, `firstName`, `lastName`, `roles`, `allAppsVisible`, `provisioningAllowed`, `visibleApps`
* `--app-fields` - [experimental] Fields to include for related apps (requires `--include visibleApps`)
* `--include` - [experimental] Include related resources: `visibleApps`
* `--visible-apps-limit` - [experimental] Maximum included visible apps (1-50; requires `--include visibleApps`)

...
* `--confirm` - Required when `--billing-mode monthly-commitment` is selected

For `monthly-commitment`, prefer the dedicated commands below. The
`--billing-mode monthly-commitment` flag on `availability edit` requires
`--confirm` and uses the same App Store Connect API 4.4
`subscriptionPlanAvailabilities` resource.
...
* Requires `--confirm` on `enable` and `disable` because both change billing
  availability; the check runs after flag validation and before any API call.

...
* `--price-point-id` - [experimental] Filter by subscription price point IDs (comma-separated)
* `--fields` - [experimental] Sparse fields for returned subscription prices
* `--territory-fields` - [experimental] Sparse fields for included territories (`currency`)
* `--price-point-fields` - [experimental] Sparse fields for included subscription price points
* `--include` - [experimental] Include `territory` and/or `subscriptionPricePoint`
* `--limit` - Results per page (1-200)
* `--paginate` - Fetch all pages
...
fails if they do not match the request.

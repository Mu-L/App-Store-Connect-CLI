...
<Note>
  Apple Pay certificates are created against a merchant ID. `asc certificates create`
  sends only the `passTypeId` relationship, so use `asc merchant-ids certificates list`
  to inspect existing Apple Pay certificates and create new ones in the Apple Developer
  portal.
</Note>

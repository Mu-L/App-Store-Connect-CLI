...
reviewed and edited before it starts building. TestFlight post-actions are not part of
the public workflow schema, so they are not copied; configure those with
...
* `--product-type` - **[experimental]** Filter by `APP` or `FRAMEWORK`
* `--fields` - **[experimental]** Sparse fields for CI products
* `--app-fields` - **[experimental]** Sparse fields for included apps; requires `--include app`
* `--bundle-id-fields` - **[experimental]** Sparse fields for included bundle IDs; requires `--include bundleId`
* `--scm-repository-fields` - **[experimental]** Sparse fields for included SCM repositories; requires `--include primaryRepositories`
* `--include` - **[experimental]** Include `app`, `bundleId`, or `primaryRepositories`
* `--primary-repositories-limit` - **[experimental]** Maximum included primary repositories (1-50); requires `--include primaryRepositories`
* `--limit` - Maximum results per page (1-200)
* `--next` - Fetch next page using a links.next URL
* `--paginate` - Automatically fetch all pages
* `--output` - Output format: `json`, `table`, `markdown`

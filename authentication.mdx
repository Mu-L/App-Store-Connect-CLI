...
<Warning>
  ...
  are never printed to stdout. Importing does not contact Apple; run
  `asc web auth status` on the target machine to confirm the session still works.
</Warning>
...
automation with `ASC_WEB_SESSION_CACHE_DIR` so they cannot read or replace the
operator's real session cache.

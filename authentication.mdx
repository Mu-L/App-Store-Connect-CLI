...
or to CI with `asc web auth export` and `asc web auth import`.
...
<Warning>
  ...
  are never printed to stdout. Import validates the bundle against Apple's
  session endpoint before writing to the cache, so the importing machine needs
  network access to Apple; an invalid, expired, unreachable, or
  identity-mismatched session is refused without changing the cache.
</Warning>

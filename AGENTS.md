...
- Within a worktree, wait for each focused test to finish before starting a broad gate. Do not overlap focused and broad tests against the same worktree or have multiple agents repeat a successful full gate for the same exact commit and unchanged worktree. Any tracked or untracked worktree change invalidates the result; rerun the relevant focused checks and full gate before claiming readiness.
- Run host-intensive gates concurrently only when explicitly required. Divide the available CPU budget with `GOMAXPROCS`, `go test -p`, and `golangci-lint --concurrency` so the combined configured concurrency does not exceed the host's logical CPU count.

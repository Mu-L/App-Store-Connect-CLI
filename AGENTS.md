...
- **IMPORTANT:** Before ending every implementation session, run `/review`. In a non-interactive environment, use `codex review --uncommitted` as a pre-commit review of working-tree changes. Before calling a PR ready, always run `codex review --base <base-branch>` on the final committed head so the complete branch or PR diff is reviewed. If the applicable review mechanism is unavailable, report the review gate as blocked and do not call the PR ready. Fix and verify every valid finding, and explicitly verify and disposition any false positive or non-actionable finding. After any change, rerun the applicable review command and repeat until it reports no actionable findings.

...
- **IMPORTANT:** Before ending every implementation session, run `/review` against the current worktree or exact PR head. In a non-interactive environment, run `codex review --uncommitted` for working-tree changes or `codex review --base <base-branch>` for the complete branch or PR diff. If neither review mechanism is available, report the review gate as blocked and do not call the PR ready. Fix and verify every valid finding, and explicitly verify and disposition any false positive or non-actionable finding. After any change, rerun the applicable review command and repeat until it reports no actionable findings.
- Do not call a PR clean, ready, complete, or express satisfaction with it until that `/review` loop is clear. Any subsequent change invalidates the clear result and requires another `/review`.

...
Push one verified private identity, certificate, and profile into a private encrypted Git repository:

```bash  theme={null}
chmod 600 \
  ~/.config/asc/signing-sync-password \
  .asc/secrets/distribution.p12 \
  .asc/secrets/distribution-p12-password
...
Run the release-testing export with the identity and ad hoc profile available only to the child process:

...
- Compare the branch with current `main` before merging. Do not rebase or merge `main` into a clean branch merely because `main` advanced; update only when an explicitly authorized merge attempt is actually refused by GitHub — an actual merge conflict, or strict up-to-date branch protection — and never bypass protection with an admin merge

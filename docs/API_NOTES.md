...
- Retry-After headers are honored when they do not exceed `ASC_MAX_DELAY`; an explicit hint above that cap fails fast with the requested delay, the cap, and guidance to raise `ASC_MAX_DELAY` rather than sleeping at the cap. Configure retry settings via `ASC_MAX_RETRIES`, `ASC_BASE_DELAY`, `ASC_MAX_DELAY`, `ASC_RETRY_LOG`.

...
- Custom Xcode Cloud version aliases are also private web-session resources. `asc web xcode-cloud settings version-aliases list` reads `GET /ci/api/teams/{publicProviderID}/products/{productID}/configuration-options/version-aliases-v3?limit=100`; `view` appends the escaped alias ID. The current UI's list response is an `items` envelope and its item fields include `id`, `name`, `type`, `locked`, `build`, `build_name`, `related_workflow_summaries`, and `build_supported`. The CLI emits only the safe scalar alias fields and deliberately omits the raw nested build and workflow payloads. The endpoint accepts a continuation offset, but its continuation response contract has not been captured, so the CLI reports at most 100 aliases and does not expose `--paginate`. Create, update, and delete remain intentionally unsupported until their live mutation behavior is verified. These endpoints are absent from the public OpenAPI specification and may change without notice.

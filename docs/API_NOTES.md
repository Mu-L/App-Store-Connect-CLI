...
- Bundle ID association submits the complete group set: the CLI reads the Bundle ID with its `bundleIdCapabilities` graph, rewrites only the `APP_GROUPS` capability's `appGroups` relationship (`enabled` follows whether the set is non-empty), and `PATCH`es `/services-account/v1/bundleIds/{id}`. `assign`, `unassign`, and `set` share this path with different computed sets; `set` and `unassign` re-read the Bundle ID afterwards and fail when the effective set differs.

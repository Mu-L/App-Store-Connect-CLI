...
- The individual creation flow is backed by the source-captured App Store Connect web bundle at `https://appstoreconnect.apple.com/access/individual-keys/0.1.9/main.52fe04d1f3ab861118ba.js`. It first reads `GET /iris/v1/users/{userUUID}` and requires a `users` resource whose `attributes.username` matches the authenticated web session's `AuthSession.UserEmail` (case-insensitive after trimming). It then reads `GET /iris/v2/apiKeys?include=createdByActor,revokedByActor&filter[createdByActor]=USER:{userUUID}` and refuses to create when an active key is already present. The user-id and username checks fail closed before any mutation.
...
- This individual-key contract is source-backed and test-covered only; no individual key was created, revoked, or otherwise mutated in Apple during capture. Revoke remains pending a separate live request capture.

...
func TestBuildEventWithContextAllowsKnownFailureParameters(t *testing.T) {
	...
	for _, parameter := range []string{"--id", "--app", "--app-id", "--key-type", "--export-xcodebuild-flag", "--active", "--availability-id", "--base-price", "--batch-id", "--copy-metadata-from", "--copyright", "--custom-app-name", "--end-date", "--group", "--input", "--keywords", "--local", "--name", "--number-of-periods", "--offer-code", "--offer-code-id", "--older-than", "--path", "--pkg", "--price-id", "--price-point-id", "--prices", "--private-key", "--product-id", "--reference-name", "--screenshot-id", "--skip-validation", "--state", "--term", "--territories", "--territory", "--tester", "--test-notes", "--treatment-id", "--type", "--upload", "--uses-non-exempt-encryption", "--uses-third-party-content", "--visible-in-app-store", "--whats-new"} {

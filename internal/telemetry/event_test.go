...
func TestBuildEventWithContextAllowsKnownFailureParameters(t *testing.T) {
	...
	for _, parameter := range []string{"--id", "--app", "--app-id", "--key-type", "--export-xcodebuild-flag", "--active", "--availability-id", "--base-price", "--batch-id", "--bg-color", "--config", "--copy-metadata-from", "--copyright", "--country", "--custom-app-name", "--demo-account-password", "--detail-fields", "--end-date", "--external-testing", "--fields", "--group", "--iap-version-fields", "--include", "--input", "--item-fields", "--keywords", "--local", "--multiplier", "--name", "--number-of-periods", "--offer-code", "--offer-code-id", "--older-than", "--path", "--pkg", "--price-id", "--price-point-id", "--prices", "--private-key", "--product-id", "--provider", "--reference-name", "--removed", "--resolved", "--response-fields", "--response-state", "--screenshot-id", "--skip-validation", "--source-subscription-id", "--stars", "--state", "--subtitle", "--subtitle-color", "--subscription-group-version-fields", "--subscription-version-fields", "--target-subscription-id", "--term", "--territories", "--territory", "--tester", "--test-notes", "--title-color", "--treatment-id", "--type", "--upload", "--uses-non-exempt-encryption", "--uses-third-party-content", "--visible-in-app-store", "--watch", "--watch-debounce", "--watch-raw-dir", "--watch-review-dir", "--whats-new", "--workers"} {

...
func capabilityRows() []Capability {
	return []Capability{
		...
		},
		{
			...
			NextAction: "Read with asc versions list --app APP_ID --paginate --output json or asc versions view --version-id VERSION_ID --output json, and write with asc versions update --version-id VERSION_ID --downloadable true|false.",
		},
		{
			Area:       "app-management",

...
// trimDeprecationWarnings drops the leading "Warning: ... is deprecated ..."
// lines a deprecated command emits before its diagnostic, so the usage-error
// assertion sees the same stderr shape for deprecated and current commands.
// Only the leading block is removed; an unexpected warning after the
// diagnostic still fails the assertion.
func trimDeprecationWarnings(stderr string) string {
	for strings.HasPrefix(stderr, "Warning: ") {
		_, rest, found := strings.Cut(stderr, "\n")
		if !found {
			return ""
		}
		stderr = rest

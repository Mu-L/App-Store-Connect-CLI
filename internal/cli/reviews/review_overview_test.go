...
func TestBuildReviewDoctorResultDisclosesWebOnlyCoverageWithoutChangingCheckSemantics(t *testing.T) {
	...
	if result.NextAction != "No public-API submission blockers detected. Run `asc web apps declarations list` before submission." {
	...
	if !strings.Contains(warning.Remediation, "asc web apps declarations list") {
		t.Fatalf("expected declarations list remediation, got %q", warning.Remediation)
	}
}

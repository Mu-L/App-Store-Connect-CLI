...
4. Never update a PR's branch preemptively. When GitHub already reports an actual merge conflict against current `main`, update the branch directly to resolve it — no merge attempt is required to prove that refusal. For a conflict-free PR, a merge attempt may only happen after every approval-and-merge gate below is satisfied and merge is explicitly authorized; make that authorized attempt without touching the branch, and update only after GitHub actually refuses it under strict up-to-date branch protection, regardless of why the base advanced. In review-only invocations, diagnose protection from read-only state and never invoke a merge. After any update (for example `gh pr update-branch <number>`), treat the result as a new head: re-verify the diff scope against current `main`, rerun `ASC_BYPASS_KEYCHAIN=1 make check-wall-of-apps` on the new exact head, and wait for required checks and mergeability before approving or merging. Never bypass branch protection with an admin merge.

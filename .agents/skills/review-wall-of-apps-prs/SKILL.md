...
4. Attempt the merge of a validated, mergeable PR without touching its branch. Update the branch only after GitHub has actually refused that merge attempt — a real merge conflict, or strict up-to-date branch protection after an earlier merge advanced `main` — never preemptively. After any update (for example `gh pr update-branch`), treat the result as a new head: re-verify the diff scope against current `main`, rerun `make check-wall-of-apps` on the new exact head, and wait for required checks and mergeability before approving or merging. Never bypass branch protection with an admin merge.

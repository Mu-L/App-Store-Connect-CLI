...
4. Never update a PR's branch preemptively. A merge attempt may only happen after every approval-and-merge gate below is satisfied and merge is explicitly authorized; make that authorized attempt without touching the branch. Update the branch only after GitHub has actually refused that authorized attempt — a real merge conflict, or strict up-to-date branch protection after an earlier merge advanced `main`. In review-only invocations, diagnose protection from read-only state and never invoke a merge. After any update (for example `gh pr update-branch`), treat the result as a new head: re-verify the diff scope against current `main`, rerun `make check-wall-of-apps` on the new exact head, and wait for required checks and mergeability before approving or merging. Never bypass branch protection with an admin merge.

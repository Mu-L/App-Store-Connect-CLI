...
Use this skill from a thread heartbeat when the same PR conversation should continue every few minutes. Each wake-up must run one pass, report only changes or blockers, and schedule or await the next pass while required checks, required reviews, or actionable reviewers remain pending. After every fix, re-fetch the pushed exact head and continue. Stop the loop only when the PR is clean, merged, closed, superseded, awaiting a material user decision, or has only advisory jobs pending. Do not create an unattended auto-merge loop.
